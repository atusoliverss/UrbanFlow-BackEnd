...
public class ReclamacaoController {
    ...
    public ResponseEntity<Page<ReclamacaoPostDto>> findByStatus(
            @RequestParam StatusReclamacao status, Pageable pageable) {
        return ResponseEntity.status(HttpStatus.OK).body(
                this.reclamacaoService.findByStatus(status, pageable).map(c -> objectMapperUtil.map(c, ReclamacaoPostDto.class))
        );
    }
    ...
    public ResponseEntity<?> update(@RequestBody @Valid ReclamacaoPutRequestDto reclamacaoPutDto) {
        return ResponseEntity.status(HttpStatus.CREATED).body(
                objectMapperUtil.map(reclamacaoService.update(objectMapperUtil.map(reclamacaoPutDto, Reclamacao.class)),
                        ReclamacaoPostDto.class)
        );
    }
}
